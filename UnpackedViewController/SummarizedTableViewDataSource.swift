...
struct SummarizedTableViewDataSource<Item> {
    ...
    static func browseAccountsDataSourceFromUser(user: User)
        -> SummarizedTableViewDataSource<Account> {
        let sections = user.brokerages.map(
            SummarizedTableViewSection<Account>.accountsSectionFromBrokerage)
        let summaryCellDataSource =
            SummaryCellDataSource.browseSummaryFromUser(user)
        return SummarizedTableViewDataSource<Account>(
            sections: sections,
            summaryCellDataSource: summaryCellDataSource)
    }
    ...
    static func accountDetailsDataSourceFromAccount(
        account: Account)
        -> SummarizedTableViewDataSource<AccountDetail> {
        let sections = [SummarizedTableViewSection<AccountDetail>
            .accountDetailsSectionFromAccount(account)]
        let summaryCellDataSource = SummaryCellDataSource
            .fromAccount(account)
        return SummarizedTableViewDataSource<AccountDetail>(
            sections: sections,
            summaryCellDataSource: summaryCellDataSource)
    }
